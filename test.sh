...
R="$(curl -s 127.0.0.1:5000/ca -X POST -d '{"request":{"method":"auth", "signatureType":"ECDSA_BLIND", "request":"1234"}, "id":"1234"}' | jq -Mc .request.token)"
...
echo "=> ECDSA signatre"
R="$(curl -s 127.0.0.1:5000/ca -X POST -d '{"request":{"method":"auth", "signatureType":"ECDSA", "request":"1234"}, "id":"1234"}' | jq -Mc .request.token)"
[ "$R" == "" ] && { echo "Error receiving signerR"; exit 1; }
hash="$(echo $RANDOM | sha256sum | awk '{print $1}')"
